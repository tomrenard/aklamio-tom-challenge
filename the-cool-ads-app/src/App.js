...
function App() {
  ...
  const updateCompanyBudget = async (company) => {
    const response = await axios.put(`http://localhost:3006/companies/${company.id}`, company);
  ...
  return (
    <>
      <GlobalStyles />
      <CompaniesList companies={companies} updateCompanyBudget={updateCompanyBudget} />
    </>
  );
}
